/**
 * How the dashboard's charts are drawn: the chart library, the line's curve and
 * the chart theme. All are saved through the settings bindings, like the
 * light or dark theme.
 */
...
import { APP_CHART_THEME } from "../../../desktop/settings.ts";
import { errorMessage } from "../api/index.ts";

type ChartSettings = Pick<
  Settings,
  "chartLibrary" | "chartCurve" | "chartTheme"
>;

const chartLibrary = ref<ChartLibrary>("vega-lite");
const chartCurve = ref<ChartCurve>("smooth");
const chartTheme = ref<string>(APP_CHART_THEME);
const saveError = ref<string | null>(null);
let bindings: DesktopBindings | null = null;

function current(): ChartSettings {
  return {
...
function apply(settings: ChartSettings) {
  ...
  chartTheme.value = settings.chartTheme;
}
...
export function useChartSettings() {
  return {
    ...
    setChartTheme: (next: string) => save({ chartTheme: next }),
  };
}
