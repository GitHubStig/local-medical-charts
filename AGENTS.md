...
- `.data/` holds the owner's real medical reports. **Don't read, print, quote or
  copy their contents** into code, tests, docs, designs, commit messages or
  chat. Don't use them for training. If a task truly needs them (e.g. testing a
  model on real scans), write a script that prints only counts and timings, and
  keep any detailed output inside `.data/`.
...
- After changing tables, the development database must be reset: delete
  `.data/medical-charts.db` with its `-wal` and `-shm` files, never all of
  `.data/`, which also holds the pipeline's reports. After changing the report
